...
# URLs das APIs
MEC_API_URL = "http://dados.gov.br/api/publico/conjuntos-dados"  # Link pode estar fora do ar
...
def json_para_pandasdf(json_data):
    """Converte lista de dicts para pandas DataFrame"""
    if not json_data:
        return None
...
def coleta_mec():
    ...
    if dados_mec:
        ...
        # O resultado pode estar em dados_mec["result"] dependendo da estrutura
        lista = dados_mec.get("result", []) if isinstance(dados_mec, dict) else dados_mec
        df_mec = json_para_pandasdf(lista)
